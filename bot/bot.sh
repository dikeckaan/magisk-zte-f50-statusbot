...
/restart_bot — Bot'u yeniden başlat
/update [all|<id>] — Modülleri GitHub'dan güncelle (check / all / specific)
...
• Sıcaklık > ${ALERT_TEMP_C}°C, RAM < %${ALERT_MEM_PCT}, tunnel düşmesi alarm
...
# Kullanım kılavuzu — performance modları arası seçimi açıklar
cmd_perf_help() {
    cat <<'EOF'
⚡ CPU/Performance Kullanım Kılavuzu
